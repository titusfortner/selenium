...
# version, regenerate CDP so the two never drift. Sets the job "output" to true on a Chrome or
# Firefox major bump, which tells CI to run the full Ruby matrix on the resulting PR.
#
...
[ -d "common/devtools/chromium/v${chrome}" ] || bazel run //scripts:update_cdp -- --chrome_channel=Stable
...
echo "output=$major" >> "$GITHUB_OUTPUT"
